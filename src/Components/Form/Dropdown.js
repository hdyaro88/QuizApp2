...
const Option = React.memo(({ isSelected, onClick, value }) => {
...
const Dropdown = React.memo(({ selected, data, setData, Options }) => {
  ...
  const ClickHandler = (value) => {
    setData((prev) => {
      return { ...prev, type: Options.indexOf(value).toString() };
    });
    setValue(value);
    setMenu(false);
  };
      ...
      <Collapse in={menu} unmountOnExit mountOnEnter>
        <div className={classes.optionBox} style={{ height: width < 600 ? "250px" : "200px" }}>
          {Options?.map((option, i) => {
            return (
              <>
                <Option isSelected={+data.type === Options.indexOf(option)} key={i} onClick={ClickHandler} value={option} />
              </>
            );
          })}
        </div>
